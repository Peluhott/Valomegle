...
function isRTCSessionDescriptionInit(value: unknown): value is RTCSessionDescriptionInit {
    ...
    return typeof candidate.type === 'string' && typeof candidate.sdp === 'string';
}
...
export class WebRTCSession {
    ...
    async handleAnswer(fromUserId: string, answer: RTCSessionDescriptionInit): Promise<void> {
        ...
        try {
            ...
            this.callbacks.onError(error instanceof Error ? error : new Error('Failed to apply remote answer'));
        }
    }
    ...
    async handleIceCandidate(fromUserId: string, candidate: RTCIceCandidateInit): Promise<void> {
        ...
        if (this.remoteDescriptionSet) {
            try {
                ...
                this.callbacks.onError(error instanceof Error ? error : new Error('Failed to add ICE candidate'));
            }
        } else {
    ...
    private resetState(peerUserId: string): void {
        ...
        pc.onicecandidate = (event) => {
            ...
            if (event.candidate && this.currentPeerUserId) {
                this.callbacks.onSendSignal(this.currentPeerUserId, 'webrtc-ice-candidate', event.candidate);
            }
        };
    ...
    private handleNegotiationFailure(error: unknown, acquiredStream: MediaStream | null): void {
        ...
        this.callbacks.onError(error instanceof Error ? error : new Error('WebRTC negotiation failed'));
        this.callbacks.onConnectionStateChange('failed');
    }
